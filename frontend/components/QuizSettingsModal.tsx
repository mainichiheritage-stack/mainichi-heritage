...
export default function QuizSettingsModal({ isOpen, onClose, heritageId = 0 }: QuizSettingsModalProps) {
      ...
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200">
        ...
        <div className="p-6 flex justify-between items-start">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-slate-400 text-sm">クイズの条件を選択してスタートしましょう</p>
          </div>
        ...
        <div className="px-8 pb-8 space-y-6">
          {/* 問題数選択 */}
          <section>
            <h3 className="text-sm font-extrabold text-slate-900 mb-4">問題数</h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                >
                  ...
                  />
                  <span className={`text-sm font-bold ${questionCount === item.value ? 'text-blue-600' : 'text-slate-700'}`}>
                    {item.label}
                  </span>
                </label>
              ))}
          ...
          {/* 難易度選択 */}
          <section>
            <h3 className="text-sm font-extrabold text-slate-900 mb-4">レベル</h3>
            <div className="grid grid-cols-2 gap-3">
              {[
                { label: '3級', value: '2' },
                { label: '2級', value: '3' },
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex flex-col">
                      <span className={`text-sm font-bold ${level === item.value && !isDisabled ? 'text-blue-600' : 'text-slate-700'}`}>
                        {item.label}
                      </span>
                      {isDisabled && (
                        <span className="text-[9px] text-slate-400 font-medium">Coming Soon</span>
                      )}
                    </div>
          ...
          <div className="flex gap-3 pt-4">
            ...
            <button 
              onClick={() => {
                onClose();
                router.push(`/quiz?count=${questionCount}&level=${level}&heritageId=${heritageId}`);
              }}
